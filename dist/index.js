...
app.get('/api/getTest', (req, res) => {
    db.query("select * , JSON_ARRAYAGG( JSON_OBJECT('userID', User.userID, 'userName', User.userName) ) as yedhee from Task left join TaskUser on Task.taskID = TaskUser.taskID left join User on TaskUser.userID = User.userID left join Team as TeamUser on User.teamID = TeamUser.teamID where Task.taskID = \"TASK-20250912-000012\" and Task.taskID in ( select taskID from TaskUser where TaskUser.userID = \"USER-2025-000002\" ) group by Task.taskID; ", (err, result) => {
        // handle err properly
        if (err) {
            console.log(err);
        }
...
app.get('/api/getAllTasksDetailed', (req, res) => {
    ...
    `, (err, result) => {
        ...
        const finalResult = result.map((row) => {
            if (row.workers) {
                const workerArray = row.workers.split(';').map((workerStr) => {
